...
class user {
    ...
    public function get_user_by_password($email, $password) {
        $query = "SELECT * FROM tbl_user WHERE `email` = '$email' AND `password` = '$password'";
        $result = $this ->db->select($query);
        return $result;
    }
    ...
    public function update_user_by_name($id, $name) {
    ...
    public function update_user_by_password($email, $new_password) {
        $query = "UPDATE `tbl_user` SET
